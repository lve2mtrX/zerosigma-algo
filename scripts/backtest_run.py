...
def main(argv: list[str] | None = None) -> int:
    ...
    c = result.counters
    m = reports.metrics(result.trades)
    ...
    print(f"  total P&L (1 contract) : ${m['total_pnl_dollars']}")
    print(f"  win rate               : {m['win_rate']}")
    print(f"  TP/SL/EOD              : {m['tp_count']}/{m['sl_count']}/{m['eod_count']}")
    print(f"  max drawdown           : ${m['max_drawdown_dollars']} (dur {m['max_drawdown_duration_trades']} trades)")
