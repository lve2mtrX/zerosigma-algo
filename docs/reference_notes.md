...
## 22. Phase 9F — Dashboard-style controls + strategy-stats aggregation (durable)

### Sandbox vs live symbol health (the fix)
`operator_mode.is_sandbox(struct, quote)` is True when structure=stub OR
quote∈{mock,null}. `symbol_health_view(..., sandbox=...)` returns "sandbox mock" /
"sandbox stub" / "sandbox eligible" (never an alarming "unavailable") in sandbox;
in live it reports real availability + a reason. This fixes the confusing screenshot
where the panel said "No ZerσSigma exposures and no Tasty market data for SPX" while
the cards below rendered stub structure.

### Dashboard-matched control CSS (values adapted from assets/zerosigma.css)
Shared in `ui_helpers.brand_css()`. Primary button = green pill
`linear-gradient(135deg,#00e5a8,#81ffd8)` / `#03130e` / radius 999px. Secondary +
restrained-danger = dark-outlined `linear-gradient(180deg, rgba(31,50,76,.92),
rgba(20,33,52,.88))` / border `rgba(88,124,182,.42)` / radius 12px (Streamlit has no
per-button "danger" kind, so the default dark button serves both). Disabled =
`opacity .42; cursor:not-allowed`. **Selectbox pill fix:** target
`.stSelectbox div[data-baseweb="select"] > div` (bg `rgba(16,24,38,.96)`, border,
radius 12px, `cursor:pointer`) + `... input { caret-color: transparent; cursor:
pointer; }` to kill the editable-text-field feel. **Limitation:** Streamlit's select
is a baseweb component (not a native `<select>`), so `appearance:none`+custom-arrow
doesn't apply; the caret/cursor are tamed, not fully replaced. Keyboard access is
preserved (we never set `pointer-events:none` on the control).

### Strategy stats from flat files (no database)
`cockpit_helpers` aggregates EXISTING outputs only, each render: `latest_run_stats`
(forward `summarize_run('latest')` + portfolio `load_latest_summary`),
`historical_stats` (sum `list_run_summaries` tick/signal/no_trade counts; per
portfolio run sum `load_closed_trades` wins/losses + `load_summary` realized/
unrealized), `common_no_trade_reasons` (count `no_trade_reason` across
`load_no_trade_log`), `latest_best_candidate` (`outputs/latest/eod_summary.json` →
`best_candidate_of_day`, often absent). All graceful when no runs exist
(`has_data=False` → "More stats will appear after additional local paper runs").
EOD JSON is NOT generated by default — only by `generate_eod_summary`.

### Phase 9G — dynamic-first presets, balanced selector, TP/SL metadata
`balanced_structure_premium_valid` (`src/selector/daily_selector.py`) evaluates BOTH
CALL_CREDIT + PUT_CREDIT among eligible/quote-valid/risk-valid rows and picks the
better side on a combined score. Implementation: two-pass — collect the eligible set,
then **min-max normalize each raw component WITHIN that set** (`_norm`: present values
→ [0,1]; None or all-equal → neutral 0.5 so a missing/flat component never dominates),
`total = Σ wᵢ·scoreᵢ − w_risk·risk_penalty`. Components emitted as
`selector_score_components` (premium_score / distance_safety_score / structure_score /
maxvol_gamma_alignment_score / quote_quality_score / existing_candidate_score /
planned_risk_penalty / total / weights / partial). Raw inputs: credit, |distance_from_
spot|, anchor_volume (→ structure_strength fallback), maxvol_alignment, bid_ask_quality
(→ quote_quality_bucket map), score, planned_stop_risk_pct. Ranking key = total → score
→ |distance|. `selector_explanation` compares the winner to the best OPPOSITE-side
runner-up. Deterministic (no clock/RNG). Default weights struct=1.0, prem=0.75,
dist=0.75, maxvol=0.75, quote=0.50, score=0.75, risk=0.50 — all configurable on
`SelectorConfig.balanced_*`.

Profile schema (`src/config/strategy_profiles.py`): the new fields are OPTIONAL with
defaults, so `from_dict` (ignores unknowns) keeps the 4 legacy profiles valid. The
`profile_hash` of legacy profiles shifts (defaults enter `hashable_dict`) but no test
pins a literal hash. Forbidden secret/execution keys still rejected.

**Deferred wiring (not faked):** per-profile TP/SL + dynamic exits do NOT touch the
paper lifecycle — `run_portfolio_forward` still constructs `PaperLifecycleConfig.from_
env()` (PAPER_* env). The cockpit states this explicitly, and `om.dynamic_exit_status`
returns "configured … not active yet" even when `dynamic_exit_enabled=true`. To wire it
