...
def test_after_hours_preview_wired():
    ...
    assert "Profile DTE" in _SRC and "Preview chain" in _SRC
...
def test_backtests_tab_discoverable_and_local_only():
    ...
    # the exact CLI text is generated by the helper, not hardcoded in Live Cockpit
    assert "scripts.backtest_run" not in _SRC
