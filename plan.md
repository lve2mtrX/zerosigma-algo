...
| **6.x — Forward runner / start-stop local paper monitoring** | **NEXT.** A start/stop multi-tick forward runner over the one-shot scanner, driven by saved run-profiles. Local paper monitoring only — still NO execution. | 🚧 next |
| **7 — Manual-confirm execution** | `manual_confirm` execution mode. Cockpit shows the order ticket; human clicks "Send" through broker UI. | ⏳ |
| **8 — Broker paper / live_tiny / live** | Real order routing, gated by explicit mode escalation. | ⏳ |
