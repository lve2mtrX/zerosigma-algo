...
| **5.x — Strategy-config persistence / forward runner** | **NEXT.** Persist resolved strategy + selector config per run; a forward (multi-tick) runner over the existing one-shot scanner. Still NO execution. | 🚧 next |
