...
| **7.x — Dashboard controls / backtest adapter** | **NEXT.** Streamlit start/stop controls for the forward runner (Phase 7.1) and/or a historical backtest adapter that replays archived snapshots through the same scanner path. Still NO live execution. | 🚧 next |
| **8 — Manual-confirm execution** | `manual_confirm` execution mode. Cockpit shows the order ticket; human clicks "Send" through broker UI. | ⏳ |
| **9 — Broker paper / live_tiny / live** | Real order routing, gated by explicit mode escalation. | ⏳ |
