...
| **8.x — Dashboard process control OR backtest adapter** | **NEXT — pick one:** (A) Streamlit start/stop process control for the forward runner (contained subprocess management + status polling), OR (B) a historical/backtest adapter that replays archived/snapped data through the same scanner path. Both still **NOT** live broker execution. | 🚧 next |
| **9 — Manual-confirm execution** | `manual_confirm` execution mode. Cockpit shows the order ticket; human clicks "Send" through broker UI. | ⏳ |
| **10 — Broker paper / live_tiny / live** | Real order routing, gated by explicit mode escalation. | ⏳ |
