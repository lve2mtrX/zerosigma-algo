...
| **10 — historical / snapped-data backtesting adapter** | **NEXT:** replay archived/snapped ZS + chain data through the EXISTING scanner path (read-only consumer of snapped data; reuses the Phase 7 in-process `run_scanner.main(argv)` seam + the Phase 7/8 forward ledger/review + the Phase 9B paper lifecycle for simulated fills). Plan + scaffold landed in Phase 9H (`docs/phase10_backtest_plan.md`, `src/replay/`, `scripts/discover_replay_data.py`); Phase 9I added `scripts/discover_backtest_sources.py` + the real-source ETL plan (§13: SPX_RAW CSVs → exposure_series, validate vs Wingonomics). Still **NOT** live broker execution, NO order placement/preview, NO snapshot *writers* (consumes existing snapshots only). | 🚧 next |
